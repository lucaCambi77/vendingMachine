...
public class ProductController {
  ...
  @Secured({MvpRoles.Code.SELLER})
  public Product addProduct(@RequestBody Product product, Authentication authentication)
      throws CoinInputException, ProductAlreadyExistsException, UserNotAuthorizedException {
  ...
  @Secured({MvpRoles.Code.SELLER})
  public Product update(@RequestBody Product product, Authentication authentication)
      throws ProductNotExistsException, UserNotAuthorizedException {
    if (hasUserPrivilegeOnProduct(authentication.getPrincipal(), product.getSellerId())) {
  ...
  @ResponseStatus(code = HttpStatus.OK)
  @Secured({MvpRoles.Code.SELLER})
  public Product delete(@RequestBody Product product, Authentication authentication)
  ...
  private boolean hasUserPrivilegeOnProduct(Object user, int product) {
    ...
    return userdetails.getAuthorities().stream()
        .noneMatch(a -> a.getAuthority().equals(Integer.toString(product)));
  }
}
