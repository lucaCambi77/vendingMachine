...
public class UserRepository {
    ...
    private final List<String> roles = Arrays.stream(MvpRoles.values()).map(MvpRoles::getAuthority).collect(Collectors.toList());
    ...
    public User mergeUser(User user) throws UserCredentialException {
        ...
        if (user.getUserRoles() == null || !roles.containsAll(user.getUserRoles().stream().map(ur -> ur.getRole().getName()).collect(Collectors.toList())))
            throw new UserCredentialException("User must have a Role defined in " + String.join(",", roles));
